...
def determine_hand_type_without_joker(cards):
...
def determine_hand_type(cards):
    joker_count = cards.count(_card_val_dict["J"])
    if joker_count == 0:
        return determine_hand_type_without_joker(cards)

    if joker_count >= 4:
        return "five_of_a_kind"

    cards_without_joker = sorted([c for c in cards if c != _card_val_dict["J"]])

    if joker_count == 3:
        if all_elements_in_list_equal(cards_without_joker):
            return "five_of_a_kind"
        else:
            return "four_of_a_kind"

    has_three_of_a_kind = False
    for sublist in get_sublists_with_size(cards_without_joker, 3):
        if all_elements_in_list_equal(sublist):
            has_three_of_a_kind = True
            break

    two_of_a_kind_count = 0
    for sublist in get_sublists_with_size(cards_without_joker, 2):
        if all_elements_in_list_equal(sublist):
            two_of_a_kind_count += 1
